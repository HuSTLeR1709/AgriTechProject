...
const Upload = ({name, label, register, errors, setValue, text}) => {
    ...
    const handelonchange = (e) => {
        ...
        useEffect(() => {
            ...
            setValue(name, product?.image)
            setimage(product?.image);
    
    }, [product?.image, setValue, name])
  ...
  <label className="text-sm text-richblack-5" htmlFor={label}>
    ...
    {text} <sup className="text-red-600">*</sup>
    </div>
  
  <div className="bg-richblack-700 flex min-h-[250px] cursor-pointer items-center justify-center rounded-md border-2 border-dotted border-green-800">
    <div
    ...
    >
      ...
      <p className="mt-2 max-w-[200px] text-center text-sm text-richblack-200">
        ...
        <span className="font-semibold text-green-800">Browse</span> a file
      </p>
...
{
                errors.courseImage && (<span className='ml-2 text-xs tracking-wide text-red-600'>
